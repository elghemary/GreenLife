    ...
    <main style={{ minHeight: "100vh", background: "#0b1c0d", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", fontFamily: "'Tajawal', sans-serif" }}>
      <div style={{ background: "#152818", border: "1px solid rgba(61,158,102,0.2)", borderRadius: "20px", padding: "40px", width: "100%", maxWidth: "400px" }}>
        <div style={{ textAlign: "center", marginBottom: "32px" }}>
          ...
          <h1 style={{ fontFamily: "'Amiri', serif", fontSize: "28px", color: "#f0e6d0", marginBottom: "6px" }}>حساب جديد</h1>
          <p style={{ color: "#9ab89a", fontSize: "15px" }}>ابدا مجاناً — بدون بطاقة</p>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          <div>
            ...
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="8 حروف على الأقل" style={inputStyle} />
          </div>
          {error && <p style={{ color: "#e07040", fontSize: "14px", textAlign: "center" }}>{error}</p>}
          <button onClick={handleSignUp} disabled={loading} style={{ background: loading ? "#2a6b42" : "#3d9e66", color: "#fff", padding: "14px", borderRadius: "12px", border: "none", fontSize: "17px", fontWeight: "700", fontFamily: "'Tajawal', sans-serif", cursor: loading ? "not-allowed" : "pointer", marginTop: "8px" }}>
            {loading ? "جاري التسجيل..." : "سجل دابا ←"}
          </button>
          <p style={{ textAlign: "center", color: "#9ab89a", fontSize: "14px" }}>
            عندك حساب؟ <Link href="/login" style={{ color: "#52c47f", textDecoration: "none" }}>دخول</Link>
          </p>
        </div>
      </div>
    </main>
